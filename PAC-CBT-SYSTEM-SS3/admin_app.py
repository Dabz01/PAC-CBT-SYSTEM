...
def _ensure_section_columns(db):
    """Best-effort migration: add section_title and section_instructions to questions table."""
    try:
        cols = {row["name"] for row in db.execute("PRAGMA table_info(questions)").fetchall()}
    except Exception:
        return
    if "section_title" not in cols:
        try:
            db.execute("ALTER TABLE questions ADD COLUMN section_title TEXT")
        except Exception:
            pass
    if "section_instructions" not in cols:
        try:
...
def result_home():
    return render_template(
        ...
        classes=["SS3 DIAMOND", "SS3 EMERALD"],
...
def result_class(klass: str):
    ...
    if klass not in ("SS3 DIAMOND", "SS3 EMERALD"):
...
def download_student_csv_template():
    klass1 = ALLOWED_CLASSES[0] if ALLOWED_CLASSES else "SS3"
