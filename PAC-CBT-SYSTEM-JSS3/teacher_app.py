...
TEACHER_ALLOWED_CLASSES = ("GRADE 9 DIAMOND", "GRADE 9 EMERALD")
...
def parse_docx_questions(docx_bytes: bytes):
    """Parse a Word (.docx) file into question objects.

    Supported markers (recommended):
      - SECTION: <text>            (optional; persists until changed)
      - INSTRUCTIONS: <text>       (optional; persists until changed)
      - QUESTION <n>: <stem>       (required per question block)
      - A. / B. / C. / D. options  (required)
      - ANSWER: A/B/C/D            (required)
      - MARKS: <int>               (optional; default 1)
      - ---                         separator between question blocks

    Notes:
      - Section/instruction lines can appear immediately before a QUESTION line.
      - Any non-marker text before a QUESTION line is ignored (prevents template headings
        from being imported as questions).
      - If a block has SECTION/INSTRUCTIONS but no QUESTION, it is treated as metadata-only
        and skipped (but the metadata carries forward to subsequent questions).
      - If a question block omits SECTION/INSTRUCTIONS, the last seen values are used.
      - If a question contains an image, the first image found in that question block is attached.
    """
    try:
    ...
    for idx, b in enumerate(blocks, start=1):
        ...
        # Track any metadata in this block (and carry forward).
        block_section = None
        block_instructions = None

        # Identify where the QUESTION line begins (if any).
        q_start = None
        for i, ln in enumerate(lines):
            if re.match(r"(?im)^SECTION\s*:\s*", ln):
                block_section = re.sub(r"(?im)^SECTION\s*:\s*", "", ln, count=1).strip() or None
                continue
            if re.match(r"(?im)^INSTRUCTIONS\s*:\s*", ln):
                block_instructions = re.sub(r"(?im)^INSTRUCTIONS\s*:\s*", "", ln, count=1).strip() or None
                continue
            if re.match(r"(?im)^QUESTION\s+\d+\s*:\s*", ln):
                q_start = i
                break

        # Update carry-forward metadata (even if block has no question)
        if block_section is not None:
            last_section = block_section
        if block_instructions is not None:
            last_instructions = block_instructions

        # Metadata-only block or noise (e.g., template headings) → skip
        if q_start is None:
            continue

        # Reconstruct the question text starting from the QUESTION line.
        q_raw = "\n".join(lines[q_start:]).strip()

        # Extract question number + remove QUESTION header
        mnum = re.search(r"(?im)^QUESTION\s+(\d+)\s*:\s*(.*)$", q_raw)
        ...
        results.append(
            {
                ...
                "section_instructions": last_instructions,
                "stem": stem,
                "options": opt,
                "correct": correct,
                "marks": marks,
...
def create_exam():
    ...
    if request.method == "POST":
        ...
        if not _is_teacher_allowed_class(exam_class):
            flash("Invalid class. Please select GRADE 9 DIAMOND or GRADE 9 EMERALD.", "danger")
            return redirect(url_for("create_exam"))
...
def questions(exam_id: int):
    ...
    if request.method == "POST":
        ...
        db.execute(
            """INSERT INTO questions (exam_id, question_text, image_path, option_a, option_b, option_c, option_d, correct_option, marks)
               VALUES (?,?,?,?,?,?,?,?,?)""",
            (exam_id, text, image_path, a, b, c, d, correct, marks)
        )
        db.commit()
...
def upload_questions_docx(exam_id: int):
    ...
    for idx, item in enumerate(parsed, start=1):
        ...
        opt = item["options"]
        db.execute(
            """INSERT INTO questions (exam_id, section_title, section_instructions, question_text, image_path, option_a, option_b, option_c, option_d, correct_option, marks)
               VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (
                exam_id,
                item.get("section_title"),
                item.get("section_instructions"),
                item["stem"],
                _save_docx_image(item.get("image")),
                opt["A"],
                opt["B"],
                opt["C"],
                opt["D"],
